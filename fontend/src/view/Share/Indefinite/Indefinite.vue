        ...
        <div class="body-content">
            <div class="header flex">
                ...
                <div class="flex">
                    ...
                    <cc-button type="primary m-r-12" icon="icon-plus-white" @click="openAddFile">Chọn tệp</cc-button>
...
import Preimage from '../../Base/Preimage.vue';
export default {
    components: {
        PopupUploadFile,
        AddFolder,
    ...
    methods: {
        ...
        getFilePersonal(){
            ...
            this.loading = true;
            FileAPI.ShareFile(param).then(res => {
                this.loading = false;
                if(res.data && res.data.Success){
        ...
        showPreviewFile(item) {
            if(item.TypeFile == "Folder") {
                ...
                // Nếu là file, hiển thị preview
                if(item.TypeFile == "Word" || item.TypeFile == "Docx") {
                    // Sử dụng Google Docs Viewer cho file Word
                    const encodedUrl = encodeURIComponent(item.Path);
                    this.srcLink = `https://docs.google.com/viewer?url=${encodedUrl}&embedded=true`;
                } else if(item.TypeFile == "Pdf") {
                    // Sử dụng trực tiếp cho file PDF
                    this.srcLink = item.Path;
                } else if(item.TypeFile == "Excel") {
                    // Sử dụng Google Docs Viewer cho file Excel
                    const encodedUrl = encodeURIComponent(item.Path);
                    this.srcLink = item.Path;
                } else {
                    // Các file khác sử dụng đường dẫn trực tiếp
                    this.srcLink = item.Path;
                }
